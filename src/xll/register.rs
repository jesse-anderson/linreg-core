...
impl Reg {
    ...
    ///   Common codes: `Q` = XLOPER12 by ref, `B` = f64, `J` = i32.
    ///   Modifiers: `$` = thread-safe, `!` = volatile.
    ...
    pub fn add(
        ...
        let mut arg_help_opers: Vec<XLOPER12> = arg_help
            .iter()
            .map(|s| XLOPER12::from_str(s))
            .collect();
        ...
        let _ = excel12(XLF_REGISTER, &mut opers);
