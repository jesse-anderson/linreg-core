...
fn main() {
    ...
    let armed = vec![
        1590.0, 1406.0, 1230.0, 1275.0, 1495.0, 1606.0, 1641.0, 1483.0, 1541.0,
        1679.0, 1704.0, 1744.0, 1869.0, 1883.0, 2089.0, 2294.0,
    ]; // Armed Forces
    ...
    let time = vec![
        ...
        1956.0, 1957.0, 1958.0, 1959.0, 1960.0, 1961.0, 1962.0,
    ]; // Year
    ...
    let result = match ols_regression(&y, &[gnp.clone(), armed.clone(), pop.clone(), time.clone()], &names) {
        Ok(r) => r,
        Err(e) => {
    ...
    println!("  F-statistic:               {:.4} (df: {}.{}, p-value: {:.4})",
        result.f_statistic, result.k - 1, result.n - result.k, result.f_p_value);
    ...
    for (i, name) in names.iter().enumerate() {
        ...
        println!("{:<16} {:>10.2} {:>10.2} {:>10.2} {:>10.4}{} {:>10}",
...
fn run_diagnostics(y: &[f64], x_vars: &[Vec<f64>]) {
    ...
    fn print_test(name: &str, stat: f64, p: f64, interpretation: &str) {
        ...
        println!("{:<25} statistic={:>8.3}  p-value={:.4}  {}", name, stat, p, status);
