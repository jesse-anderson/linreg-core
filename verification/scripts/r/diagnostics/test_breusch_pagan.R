# ============================================================================
# Breusch-Pagan Test Reference Implementation (R)
# ============================================================================
# This script generates reference values for the Breusch-Pagan test using R's
# lmtest::bptest function. The Breusch-Pagan test checks for heteroscedasticity
# by regressing squared residuals on the original predictors.
#
# Source: lmtest package, bptest function
# Reference: Breusch & Pagan (1979), "A Simple Test for Heteroscedasticity
#            and Random Coefficient Variation"
#
# Usage:
#   Rscript test_breusch_pagan.R [csv_path] [output_dir]
#   Args:
...
# Run Breusch-Pagan test with studentize=TRUE (R's default)
bp_result <- bptest(model, studentize = TRUE)

# Print results
cat("Breusch-Pagan Test (R - lmtest::bptest)\n")
cat("=======================================\n")
cat("Dataset:", dataset_name, "\n")
cat("Formula:", formula_str, "\n")
cat("LM-statistic:", bp_result$statistic, "\n")
cat("p-value:", bp_result$p.value, "\n")
cat("Passed:", bp_result$p.value > 0.05, "\n\n")

# Prepare output
output <- list(
  test_name = "Breusch-Pagan Test (R - lmtest::bptest)",
  dataset = dataset_name,
  formula = formula_str,
  statistic = as.numeric(bp_result$statistic),
  p_value = as.numeric(bp_result$p.value),
  passed = bp_result$p.value > 0.05,
  description = "Tests for heteroscedasticity by regressing squared residuals on predictors. R variant uses lmtest::bptest with studentize=TRUE."
)
...
output_file <- file.path(output_dir, paste0(dataset_name, "_breusch_pagan.json"))
