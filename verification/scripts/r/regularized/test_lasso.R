# ============================================================================
# Lasso Regression Test Reference Implementation (R with glmnet)
# ============================================================================
# This script generates reference values for lasso regression using R's
# glmnet package with alpha=1. The test validates that the Rust implementation
# matches glmnet's behavior for coefficient paths and predictions.
#
# Source: glmnet package, glmnet function with alpha=1
# Reference: Friedman, Hastie, Tibshirani (2010), "Regularization Paths for
#            Generalized Linear Models via Coordinate Descent"
#
# Usage:
#   Rscript test_lasso.R [csv_path] [output_dir] [lambda_count]
#   Args:
...
convert_categorical_to_numeric <- function(data, dataset_name) {
  ...
  if (length(non_numeric_cols) > 0) {
    cat(paste0("INFO: Dataset '", dataset_name, "' contains non-umeric columns: ",
...
cat(sprintf("Running lasso regression test on dataset: %s\n", dataset_name))
...
# Add dummy predictor if only 1 predictor (glmnet requires 2+ columns for lasso)
# Use a column of zeros as dummy (doesn't affect regularization test)
if (p < 2) {
  cat("  INFO: Adding dummy predictor (zeros) for lasso regression\n")
  X <- cbind(X, rep(0, n))
  colnames(X) <- c(colnames(X)[1], "dummy")
  p <- ncol(X)
}

# Run glmnet for lasso regression (alpha = 1)
...
# Count non-zero coefficients for each lambda (excluding intercept)
nonzero_counts <- colSums(coef_matrix[-1, , drop = FALSE] != 0)
...
result <- list(
  ...
  nonzero_counts = as.vector(nonzero_counts),
...
# Write output (use _lasso_glmnet suffix to match test expectations)
output_file <- file.path(output_dir, paste0(dataset_name, "_lasso_glmnet.json"))
