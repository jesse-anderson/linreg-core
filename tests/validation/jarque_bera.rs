// ============================================================================
// Jarque-Bera Test Validation
// ============================================================================
//
// Comprehensive validation of the Jarque-Bera normality test
// against R and Python reference implementations.
//
// The Jarque-Bera test checks if residuals are normally distributed
// by examining skewness and kurtosis.
...
#[test]
fn validate_jarque_bera_all_datasets() {
    println!("\n");
    println!("╔══════════════════════════════════════════════════════════════════════╗");
    println!("║  JARQUE-BERA TEST - COMPREHENSIVE MULTI-DATASET VALIDATION         ║");
    println!("╚══════════════════════════════════════════════════════════════════════╝");
    println!();
    ...
    for dataset_name in ALL_DATASETS {
        ...
        let rust_result =
            match diagnostics::jarque_bera_test(&dataset.y, &dataset.x_vars) {
                Ok(r) => r,
                Err(e) => {
                    println!("     Jarque-Bera test failed: {}", e);
                    failed_tests.push((
                        dataset_name.to_string(),
                        format!("JB test error: {}", e),
                    ));
                    continue;
                },
            };
        ...
        println!(
            "    Rust: X-squared = {:.6}, p = {:.6}",
            rust_result.statistic, rust_result.p_value
        );

        // Validate against R
        let r_result_path = r_results_dir.join(format!("{}_jarque_bera.json", dataset_name));
        if let Some(r_ref) = load_r_diagnostic_result(&r_result_path) {
            ...
            println!("    R:        X-squared = {:.6}, p = {:.6}", r_stat, r_pval);
            println!(
                "              Diff: stat = {:.2e}, p = {:.2e}",
                stat_diff, pval_diff
            );
        ...
        let python_result_path =
            python_results_dir.join(format!("{}_jarque_bera.json", dataset_name));
        if let Some(py_ref) = load_python_diagnostic_result(&python_result_path) {
            ...
            println!("    Python:   X-squared = {:.6}, p = {:.6}", py_stat, py_pval);
            println!(
                "              Diff: stat = {:.2e}, p = {:.2e}",
                stat_diff, pval_diff
            );
    ...
    // Summary
    println!("╔══════════════════════════════════════════════════════════════════════╗");
    println!("║  JARQUE-BERA VALIDATION SUMMARY                                    ║");
    println!("╠══════════════════════════════════════════════════════════════════════╣");
    ...
    assert!(total_tests > 0, "No Jarque-Bera validation tests were run.");
    let pass_rate = (passed_r + passed_python) as f64 / total_tests as f64;
    assert!(
        pass_rate >= 0.9,
        "Jarque-Bera validation pass rate ({:.1}%) is below 90%.",
        pass_rate * 100.0
    );

    println!();
    println!(" Jarque-Bera comprehensive validation passed!");
}
