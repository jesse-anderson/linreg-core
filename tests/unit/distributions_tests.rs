...
use linreg_core::distributions::{
    ln_gamma, inc_beta, inc_gamma_upper,
    student_t_cdf, fisher_snedecor_cdf, chi_squared_survival,
    normal_cdf_cephes, normal_sf_cephes,
    normal_inverse_cdf, student_t_inverse_cdf,
    cephes_erf, cephes_erfc, cephes_erfce,
};
use linreg_core::core::{two_tailed_p_value, f_p_value};
...
const TOL_LOOSE: f64 = 1e-6; // For iterative methods
    ...
    fn test_normal_inverse_cdf_roundtrip() {
        let ps = [
            1e-12, 1e-10, 1e-8, 1e-6, 1e-4, 1e-3,
            0.01, 0.1, 0.25, 0.5, 0.75, 0.9, 0.99,
            1.0 - 1e-3, 1.0 - 1e-4, 1.0 - 1e-6, 1.0 - 1e-8, 1.0 - 1e-10, 1.0 - 1e-12,
        ];
    ...
    fn test_normal_inverse_cdf_monotonicity_and_symmetry() {
        ...
        let ps = [
            ...
            1.0 - 1e-3, 1.0 - 1e-6, 1.0 - 1e-8, 1.0 - 1e-10, 1.0 - 1e-12,
        ];
        ...
        for &p in &ps {
            ...
            assert!(z > last, "monotonicity failed: inv({})={} <= last={}", p, z, last);
            last = z;
    ...
    fn test_normal_cdf_cephes_symmetry_and_bounds() {
        let zs = [
            -10.0, -8.0, -6.0, -4.0, -3.0, -2.0, -1.0, -0.5, 0.0,
    ...
    fn test_inc_beta_complement_and_monotonicity() {
        ...
        for &(a, b) in &params {
            ...
            // Monotonic in x
            let xs = [1e-12, 1e-6, 1e-3, 0.01, 0.1, 0.5, 0.9, 0.99, 1.0 - 1e-6, 1.0 - 1e-12];
            let mut last = -1.0;
            for &x in &xs {
                let v = inc_beta(x, a, b);
                assert!(v >= 0.0 && v <= 1.0, "inc_beta out of bounds: a={},b={},x={},v={}", a,b,x,v);
                assert!(v >= last, "inc_beta not monotone: a={},b={}, x={}, v={}, last={}", a,b,x,v,last);
                last = v;
            }
        }
    }
    ...
    fn test_inc_gamma_upper_monotonicity() {
        ...
        for &a in &as_ {
            let xs = [0.0, 1e-12, 1e-6, 1e-3, 0.01, 0.1, 1.0, 2.0, 5.0, 10.0, 25.0, 50.0];
            let mut last = 1.0;
    ...
    fn test_normal_cdf_bounds_and_monotone() {
        ...
        for &z in &zs {
            ...
            assert!(p >= last, "Phi not monotone: z={}, p={}, last={}", z, p, last);
            last = p;
        }
    }
    ...
    fn test_normal_cdf_sf_complement() {
        for &z in &[-10.0, -6.0, -2.0, 0.0, 2.0, 6.0, 10.0] {
            ...
            assert!((p + q - 1.0).abs() < 1e-14, "z={}, p={}, q={}, sum={}", z, p, q, p + q);
        }
    }
